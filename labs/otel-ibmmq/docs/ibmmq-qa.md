...
## Does a traceparent generated by an API exit represent a full distributed trace? `#o11y`

No — and that is the core limitation of the exit approach.

The exit gives you a trace that starts at the MQ boundary:

```
[nothing] → MQPUT → validator → enricher → processor
              ↑
         exit generates traceparent here
```

A full distributed trace should look like this:

```
HTTP request → gateway → MQPUT → validator → enricher → processor
     ↑
 trace origin — traceparent and baggage born here, carried through every hop
```

The baggage and `traceparent` that the OTel SDK injects carry context from the
very first entry point — the HTTP request — all the way through the MQ boundary
to the last consumer. Every span in the chain shares the same trace ID and has
`bsi.ep`, `bsi.ch`, `bsi.cj` as attributes.

The exit's generated `traceparent` is a stub — it connects the MQ hops to each
other but severs the link to everything that happened before `MQPUT`. You cannot
answer:

- Which HTTP request triggered this processor span?
- How long did the full journey take from the user's request to final processing?
- Which customer journey (`bsi.cj`) caused this DLQ message?

Those questions require context that originates at the HTTP layer and flows
unbroken through every hop. That is exactly what W3C Baggage is designed for —
and exactly what the exit cannot provide because it only sees the MQ API call,
